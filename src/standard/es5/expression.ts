...
export const Expression = {
    UnaryExpression(path) {
        log('UnaryExpression', path)
        // 一元操作符表达式: - + ! ~ viod typeof delete
        // 一元操作符表达式由操作符和参数组成, 参数用 node.argument 表示
        ...
        return unaryOperatorMap[node.operator]();
    },
    BinaryExpression(path) {
        log('BinaryExpression', path)
        const { node } = path;
        // 对两个参数进行求值, 然后使用操作符
